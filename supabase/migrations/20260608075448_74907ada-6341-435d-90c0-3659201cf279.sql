...
DECLARE
  ...
  is_google boolean := (app_meta->>'provider' = 'google') OR ((app_meta->'providers') ? 'google');
  email_known boolean := EXISTS (
    SELECT 1 FROM public.profiles WHERE LOWER(email) = LOWER(NEW.email)
  );
BEGIN
  -- Anti-pollution: Google sign-ins by non-members must NOT create profile/role rows.
  -- They will be signed out and the auth user removed by the cleanup edge function.
  IF is_google AND NOT is_org AND NOT email_known THEN
    RETURN NEW;
  END IF;
